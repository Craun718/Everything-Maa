...
5. Run `python -m pytest`, `python scripts/validate_skills.py`, `npm test`, and `npm pack --dry-run` before submitting a change. Integration changes must also pass the relevant `npm run smoke:create-project` or `npm run smoke:maafw-cli` upstream contract test.
