...
3. In the npm package settings, configure GitHub Actions as the trusted publisher:
   - owner: `KhazixW2`;
   - repository: `Everything-Maa`;
   - workflow filename: `release.yml`;
   - allowed action: npm publish.
4. Protect the default branch and release tags. Optionally attach an approval-protected GitHub environment before enabling public releases.
...
The Release workflow re-runs validation, creates the tarball, publishes it through npm trusted publishing, and then creates a GitHub Release with that exact tarball. A manual workflow dispatch builds and uploads the artifact but deliberately does not publish.
